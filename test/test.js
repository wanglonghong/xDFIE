const ico = artifacts.require("MainContract");
const dfie = artifacts.require("DFIEToken");
...
contract('ICO test', async (accounts) => {
	it("shoud init init with contract balance of 1000 tokens", async () => {
        let dfie = await ico.deployed();
		let ins = await ico.deployed(dfie.address);

		let bal = await dfie.balanceOf(dfie.address);

		assert.equal(bal, 10000000);
	})
